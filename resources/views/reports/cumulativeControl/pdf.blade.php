...
<head>
    ...
    <title>Control Acumulativo</title>
</head>
<body>
<div id="header">
    <figure>
    ...
    <table>
        @foreach($data as $key => $location)
            ...
            @foreach($location['additions'] as $addition)
                <tr>
                    <td class="contentCol1">{{ $addition->concept->code }}</td>
                    <td class="contentCol2">{{ $addition->concept->name }}</td>
                    @if($estimate->contract->type == 2 )
                        <td class="contentCol3">%</td>
                    @else
                        <td class="contentCol3">{{ $addition->concept->measurement_unit }}</td>
                    @endif

                    <td class="contentCol4">{{$addition->concept->quantityOk }}</td>
                    <td class="contentCol4">{{ $addition->lastQuantityOk }}</td>
                    <td class="contentCol4">{{ $addition->quantityOk }}</td>

                    <td class="contentCol4">{{ $addition->accumulatedQuantityOk }}</td>

                    <td class="contentCol5">{{ $addition->concept->unitPriceOk }}</td>



                    <td class="contentCol5">{{ $addition->lastAmountOk }}</td>
                    <td class="contentCol5">{{ $addition->amountOk }}</td>
                    <td class="contentCol5">{{ $addition->accumulatedAmount }}</td>
                </tr>
            @endforeach
            <tr>
            ...
            @if(count($location['deductions']) > 0)
                ...
                @foreach($location['deductions'] as $deduction)
                    <tr>
                        <td class="contentCol1">{{ $deduction->concept->code }}</td>
                        <td class="contentCol2">{{ $deduction->concept->name }}</td>
                        @if($estimate->contract->type =='2')
                            <td class="contentCol3">%</td>
                        @else
                            <td class="contentCol3">{{ $deduction->measurement_unit }}</td>
                        @endif
                        <td class="contentCol4">{{ $deduction->concept->quantityOk }}</td>
                        ...
                        <td class="contentCol5">{{ $deduction->concept->unitPriceOk }}</td>
